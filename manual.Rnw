...
Performing the Jonckheere-Terpstra test in R requires the \CRANpkg{clinfun} package to be installed (Section \ref{installPackages}) (we recommend using this version of the test rather than versions in other packages).
...
\item{\textbf{Logistic regression} \textendash when the response variable is a nominal (or categorical) variable}
\end{itemize}
...
\begin{table}[!htb]
\caption{Which test to use?}
\vspace{0.6cm}
%\begin{center}
%\begin{tabular}{p{2cm}|p{2cm}p{2cm} p{2cm}|p{2cm} }
\begin{tabular}{c|c|c|c|c}
\hline
\multicolumn{2}{c}{\bf Groups} & \multicolumn{3}{|c}{ \bf Variable} \\  \hline
% \multicolumn{2}{c} {\bf /Independent} & \multicolumn{3}{|c}{ \bf } \\  \hline 
\bf N.  & {\bf Independent }  &  {\bf Nominal or } &  {\bf Ordered categorical or} & {\bf Continuous and } \\  
\bf  samples  & {\bf (unpaired) Y/N }  &  {\bf  categorical} &  { \bf continuous and non normal} & {\bf normally distributed} \\  \hline
  % \multicolumn{2}{|} {} & \multicolumn{1}{|}{categorical} & \multicolumn{1}{|}{ continuous and non normal} & \multicolumn{1}{|}{normally distributed} \\  \hline 
One   &  N/A & Sign test  & Sign test & t-test  \\
 sample  &   & (binom.test) & (binom test) & (t.test)\\ 
 &    &  $\chi^2$   test   &  Kolmogorov-Smirnov test &   \\
   &    & (chisq.test)  & (ks.test)    & \\ \hline \hline
 Two  & Yes  & $\chi^2$ test(2xK)&Mann Whitney rank sum test &Unpaired t-test  \\
sample &&(chisq.test)&(wilcox.test)& (t.test)\\ 
&&Fishers exact test&Median test\\ 
&&(fisher.test)&(chisq.test)&  \\ \cline{2-5}
 &No&McNemars test&Wilcoxon signed rank test&Paired t-test \\
 &&(McNemar.test)&(Wilcox.test)&   (t.test) \\  
 & & & Sign test & \\
  & & & (binom.test) & \\
  \hline \hline
Multiple&Yes&$\chi^2 $(r x k) &Kruskal-Wallis (Section \ref{kruskalWallis}) &Analysis of variance (ANOVA) (Section \ref{oneWayAnova}) \\
Samples & &(chisq.test)& (kruskal.test) & (Aov) \\
((k>2)  &&&Median test (Section \ref{medianTest}) &\\
&&& (chisq.test)&\\
& Yes  (ordered) & & & Jonckheere-Terpstra (trend) test (Section \ref {jt-test})\\ 
& & & & (jonckheere.test  ) \\ \cline{2-5}
&No&Cochran Q-test &Friedman test (Section \ref{friedman}) &Repeated measures ANOVA \\
&&(not covered in course)&(friedman.test)& (aov) \\  \hline 
% \multicolumn{2}{c}{\bf Association between two variables} & contingency coefficient & Spearman's rank correlation & Pearson's product moment correlation \\
