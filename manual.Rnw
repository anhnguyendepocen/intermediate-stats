...
install.packages(c("ggplot2", "reshape2","gridExtra","RColorBrewer","knitr","beeswarm"))
...
We can make a similar assessment of the data using a histogram (plotted separately for each group in our dataset), and in particular we can use our histograms to make an assessment of normality and constant variance which are two of the assumptions behind ANOVA. See Figure \ref{fig:hist1}
...
summary(aov(mod))
@
One of the assumptions of the one-way ANOVA, which we have already explored with our scatter plot, is that the variances in each of the five groups are approximately equal. GraphPad Prism also formally tests this assumption when carrying out a one-way ANOVA, using a Bartlett's test. The results provide a p-value indicating whether equal variance can be assumed, with a value less than 0.05 suggesting that equal variance should not be assumed. 
...
In this example, the p-value was \Sexpr{round(bt$p.value,3)}, so there is no evidence to suggest that the variances in each of the five groups aren't approximately equal to each other. It is important to note than GraphPad Prism does not perform the Bartlett's test if any of the treatment groups have fewer than 5 observations. If the Bartlett's test gives a significant p-value, we cannot assume equal variances across the groups and a non-parametric test, such as the Kruskal-Wallis test, should be used instead of a one-way ANOVA.
