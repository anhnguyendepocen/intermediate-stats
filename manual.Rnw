...
\item{3. Rank each persons' score from lowest to highest and sum ranks in each column.}
...
Performing the Jonckheere-Terpstra test in R requires the \CRANpkg{clinfun} package to be installed (\ref{installPackages}).
...
If the assumptions of the Kruskal-Wallis test (\ref{kruskalWallis}) are satisfied, it is more powerful to use this test than the Median Test (\ref{medianTest}).  That is the Kruskal-Wallis test (\ref{kruskalWallis}) is more likely to correctly reject the null hypothesis.  
...
Coefficient, residuals and other properties of the model can be obtained using a number of useful functions (see box) or accessed directly through the model object:
...
These sources of variation are laid out in an analysis of variance table in T  able \ref{regression-anova-table}. The mean squares column contains values for the variance for each source, calculated as:
...
The output includes the standard errors for both the concentration coefficient (slope) and intercept.
...
Confidence intervals on the model coefficients are calculated from the $t$-value and the standardd error. Recall that in Student's $t$-distribution, values of $t$ are the numbers of standard errors to be expected with specified probability and for a given number of degrees of freedom.
...
\item{\textbf{Independence of errors} \textendash\ the errors in the response variables are uncorrelated with other}
