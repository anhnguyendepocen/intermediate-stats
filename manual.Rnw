...
library(xtable)
messy <- data.frame(Name = c("John Smith","Jane Doe","Mary Johnson"),treatmenta = c("-",16,3),treatmentb=c(2,11,1))
...
messy <- data.frame(Name = c("John Smith","Jane Doe","Mary Johnson"),
                    treatmenta = c(NA,16,3),treatmentb=c(2,11,1))
@
...
To see how to compute the statistic in R, we first have to re-format into a more convenient format using the \Rcode{gather} function from the package \CRANpkg{tidyr} package. We can then use the \Rcode{kruskal.test} function, which applies the kruskal-wallis test.
...
\subsubsection{Example}
The data for this example is taken from Rubin and Peter's paper.  The Friedman test will be used to study whether or not hydralazine would relieve high blood pressure in the lungs.
...
Tests whether two or more independent samples are drawn from populations with the same median using the $\chi^2$ statistic.  It can be used when the assumptions of similarity of distributions for the Mann-Whitney U and Kruskal-Wallis tests are not met.
...
The median test is implemented in the \CRANpkg{RVAideMemoire} package. As usual we need to tidy the input data frame in order to define a factor for the test. The \Rfunction{mood.medtest} function implements the test and has the option to compute an exact, or approximate p-value.
<<>>=
...
Although not strictly necessary, we now use \Rfunction{attach} to make the columns of the data frame accessible by name:
...
and we can generate something similar to figure \ref{fig:scatterplot} using the \Rfunction{plot} function:

<<eval=FALSE>>=
plot(conc, growth, pch=16, xlab="Concentration", ylab="Growth rate")
@
...
plot(conc, growth, pch=16, xlab="Concentration", ylab="Growth rate")
model <- lm(growth ~ conc)
abline(model)
fitted <- predict(model)
for (i in 1:10) lines(c(conc[i], conc[i]), c(growth[i], fitted[i]))
@
...
<<echo=FALSE>>=
error_variance <- SSE / (length(growth) - 2)
...
\hline
Error & $SSE = \Sexpr{SSE}$ & \Sexpr{length(growth) - 2} & $s^2 = \Sexpr{error_variance}$ & \\
\hline
Total & $SSY = \Sexpr{SSY}$ & \Sexpr{length(growth) - 1} & & \\
\hline
\end{tabular}
